...
void Random(std::vector<int> &ToShuffle, const int &HowManyShuffles, const int& RowNumber) {
    srand(time(0));
...
public:
    ...
    FieldPosition(std::vector<int> &ToCreate, const int& RowNumber);
    FieldPosition(std::vector<int> &ToCreate, const int &NewLevel, const std::vector<int> &Parent, const int& RowNumber);
...
GameSolver::GameSolver(const int& RowNum){
    RowNumber = RowNum;
    FieldSize = RowNum * RowNum;
}
...
int FieldPosition::ConflictLinear(){
    ...
    for(int i = 0; i< RowNumber; ++i)
    {
        for(int first = 0; first < RowNumber; ++first)
        {
            ...
            if(Position[first*RowNumber + i] % RowNumber == (i + 1) % RowNumber)
            {
                for(int second = first + 1; second < RowNumber; ++second)
                {
...
bool FieldPosition::Find(const int &ToFind) {
...
FieldPosition::FieldPosition(std::vector<int> &ToCreate, const int &NewLevel, const std::vector<int> &Parent, const int& RowNum) {
    RowNumber = RowNum;
    FieldSize = RowNum * RowNum;
    Position = ToCreate;
    Level = NewLevel;
...
FieldPosition::FieldPosition(std::vector<int> &ToCreate, const int& RowNum) {
    RowNumber = RowNum;
    FieldSize = RowNum * RowNum;
    Position = ToCreate;
    Level = 0;
...
FieldPosition FieldPosition::MakeNewNeighbor(const int &WhereIsNull, const short &Shift) const {
    ...
    FieldPosition Neighbor = FieldPosition(NeighborPosition, this -> Level + 1, this -> Position, RowNumber);
    return Neighbor;
}
...
bool FieldPosition::HasSolution() {
    int Result = 0;
    std::vector<int> StartPosition = this -> Position;
...
void FieldPosition::CheckNewPosition(FieldPosition &Minimal, std::set<FieldPosition> &Opened, std::map<std::vector<int>, std::pair<int, std::vector<int>>> &DatMap) {
    ...
    if(OurElement != DatMap.end()) {
        ...
        if((jo = GetWeight()) < OldDistance) {
            FieldPosition ToDelete = FieldPosition(Position, OurElement -> second.first - CountDistance(), Minimal.Position, RowNumber);
...
int main(int argc, const char * argv[]) {
    ...
    for(int j = 0; j < GameMode; ++j) {
        ...
        FieldPosition StartField = FieldPosition(StartPosition, RowNumber);
        
        if(!StartField.HasSolution()) {
            std::cout << "This puzzle is not solvable.\n";
            continue;
        }
        
        GameSolver SolveMaster(RowNumber);
        
        std::vector<std::string> Answer;
        auto Start = std::chrono::steady_clock::now();
        //        Answer = SolveMaster.Run(StartField);
