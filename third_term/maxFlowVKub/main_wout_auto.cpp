...
void Net::Lift(const size_t &from) {
    ...
    for(int pos = 0; pos < listOfEdges[from].size(); ++pos) {
        if(edges[listOfEdges[from][pos]].capacity - edges[listOfEdges[from][pos]].flow > 0)
            minimalHeight = std::min(minimalHeight, height[edges[listOfEdges[from][pos]].to]);
    }
...
void Net::Discharge(const size_t &from) {
    while (excess[from] > 0) {
        long long to = positionInSequence[from];
...
void Net::RelabelToFront() {
    for(int to = 0; to < listOfEdges[source].size(); ++to) {
        if(edges[listOfEdges[source][to]].capacity != 0)
            Push(edges[listOfEdges[source][to]], (listOfEdges[source][to] % 2 == 0)? edges[listOfEdges[source][to] + 1]:edges[listOfEdges[source][to] - 1]); //tk rebra dobavlyayutsa po 2
    }
    for(long long to = source + 1; to < vertexNumber - 1; ++to)
        vertexQueue.push_back(to);
    
    auto currentPosition = vertexQueue.begin();
    while(currentPosition != vertexQueue.end()) {
        long long thisNode = *currentPosition;
        long long oldHeight = height[thisNode];
        Discharge(thisNode);
    ...
    for(int edge = 0; edge < edges.size(); ++edge) {
        if(edges[edge].to == vertexNumber - 1) {
            maxFlow += edges[edge].flow;
        }
    }
    for(int i = 0; i < edges.size(); i += 2)
        edgeFlows.push_back(edges[i].flow);
    std::cout << maxFlow << "\n";
    for(int flow = 0; flow < edgeFlows.size(); ++flow)
        std::cout << edgeFlows[flow] << "\n";
...
//    std::ios_base::sync_with_stdio(false);
    ...
    for(size_t i = 0; i < edgesNumber; ++i) {
        ...
        Edge thisEdge = {from - 1, to - 1, capacity, 0}, reverseEdge = {to - 1, from - 1, 0, 0};
        thisEdges.push_back(thisEdge);
        thisEdges.push_back(reverseEdge);
